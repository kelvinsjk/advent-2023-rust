...
pub fn solution() {
	...
	for line in input.lines() {
		...
		let mut x = map.to_string();
		let mut y = reqs.to_string();
		for _ in 0..4 {
			x.push('?');
			x.push_str(map);
			y.push(',');
		...
		let reqs: Vec<usize> = y.split(',').map(|x| x.parse::<usize>().unwrap()).collect();
...
fn solutions(row: &Vec<char>, reqs: &Vec<usize>) -> usize {
	...
	let mut mid = row.len() / 2;
	while row[mid] == '#' {
		mid += 1;
	}
	let req_len = reqs.len();
	let mut sum = 0;
	for i in 0..req_len + 1 {
		sum += solutions(&row[..mid].to_vec(), &reqs[0..i].to_vec())
			* solutions(&row[mid + 1..].to_vec(), &reqs[i..req_len].to_vec());
	}
	if row[mid] == '.' {
		return sum;
	} else if row[mid] == '?' {
		...
		new_row_hit[mid] = '#';
		return sum + solutions(&new_row_hit, &reqs);
	}
	sum
