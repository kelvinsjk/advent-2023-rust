...
fn score(input: &str) -> [u8; 6] {
	...
	let number_of_js = map.get(&'J').unwrap_or(&0);
	let overall_score: u8 = match number_of_js {
		5 => 7, // five of a kind
		4 => 7, // five of a kind
		3 => {
			match map.len() {
				3 => 6, // 3Js, 1, 1 => four of a kind
				2 => 7, // 3Js, 2 => five of a kind
				_ => panic!("Unexpected map length"),
			}
		}
		2 => {
			match map.len() {
				2 => 7, // 2Js, 3 => five of a kind
				3 => 6, // 2Js, 2, 1 => four of a kind
				4 => 4, // 2Js, 1, 1, 1 => three of a kind
				_ => panic!("Unexpected map length"),
			}
		}
		1 => {
			match map.len() {
				...
				3 => {
					...
					match max_pair {
						2 => 5, // J, 2, 2 => full house
						3 => 6, // J, 3, 1 => four of a kind
						_ => panic!("Unexpected count for map of length 3"),
					}
				}
				2 => 7, // J, 4 => five of a kind
				_ => panic!("Unexpected map length"),
			}
		}
		0 => default_score(map),
		_ => panic!("Unexpected number of Js"),
	};
...
fn default_score(map: HashMap<char, u8>) -> u8 {
	match map.len() {
		5 => 1, // high card
		4 => 2, // one pair
		3 => {
